...
import { EmptyCategoryState } from "./category-empty-state"
      ...
      ...(data?.events[0]
        ? Object.keys(data.events[0].fields as object).map((field) => ({
            accessorFn: (row: Event) => {
              const fieldValue = (row.fields as Record<string, any>)[field]
              // Convertir el objeto a string si es necesario
            ...
            cell: ({ row }: { row: Row<Event> }) => {
              const fieldValue = (row.original.fields as Record<string, any>)[
                field
              ]
              return typeof fieldValue === "object"
                ? JSON.stringify(fieldValue)
